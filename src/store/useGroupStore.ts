...
import { createSharedGroupFile, readSharedGroupFile, writeSharedGroupFile, deleteSharedGroupFile } from '@/services/googleSync'
...
interface GroupState {
  ...
  addGroup: (name: string, description?: string, currency?: string) => Promise<Group>
  ...
  updateGroupExpense: (groupId: string, expenseId: string, data: Partial<GroupExpense>) => Promise<void>
  deleteGroupExpense: (groupId: string, expenseId: string) => Promise<void>
  settleUp: (groupId: string, expenseId: string, memberId: string, settledBy?: string) => Promise<void>
  unsettle: (groupId: string, expenseId: string, memberId: string) => Promise<void>
...
export const useGroupStore = create<GroupState>((set, get) => ({
  ...
  loadGroupExpenses: async (groupId) => {
    ...
    set(s => ({ groupExpenses: { ...s.groupExpenses, [groupId]: expenses } }))
  },

  addGroup: async (name, description, currency = 'USD') => {
    const group = await groupQueries.add({ name, description, members: [], currency })
  ...
  deleteGroup: async (id) => {
    ...
    set(s => {
      ...
      return { groups: s.groups.filter(g => g.id !== id), groupExpenses: rest }
    })
  },

  addMember: async (groupId, name, email) => {
    const member: GroupMember = { id: generateId(), name, email, avatarColor: avatarColor(name) }
  ...
  removeMember: async (groupId, memberId) => {
    ...
    set(s => ({ groups: s.groups.map(g => g.id === groupId ? { ...g, members } : g) }))
  },
  ...
  deleteGroupExpense: async (groupId, expenseId) => {
    ...
    set(s => ({
      groupExpenses: {
        ...s.groupExpenses,
        [groupId]: (s.groupExpenses[groupId] ?? []).filter(e => e.id !== expenseId),
      },
    }))
  },
  ...
  settleUp: async (groupId, expenseId, memberId, settledBy?: string) => {
    ...
    const splits = expense.splits.map(s =>
      s.memberId === memberId
        ? { ...s, settled: true, settledAt: now, settledBy: settledBy ?? memberId }
        : s
    )
    ...
    set(s => ({
      groupExpenses: {
        ...s.groupExpenses,
        [groupId]: expenses.map(e => e.id === expenseId ? { ...e, splits } : e),
      },
    }))
  },
  ...
  shareGroup: async (groupId) => {
    ...
    try {
      const expenses = get().groupExpenses[groupId] ?? await groupExpenseQueries.getByGroup(groupId)
      ...
      set(s => ({ groups: s.groups.map(g => g.id === groupId ? updated : g), syncingGroupId: null }))
      return fileId
    } catch (e) {
  ...
  joinGroup: async (shareCode) => {
    ...
    try {
      const payload = await readSharedGroupFile(shareCode) as { group: Group; expenses: GroupExpense[] } | null
      if (!payload?.group) throw new Error('Invalid share code or group not found')
      ...
      await groupQueries.update(group.id, group).catch(async () => {
        await groupQueries.add({ name: group.name, description: group.description, members: group.members, currency: group.currency })
      })
      ...
      set(s => ({
        groups: s.groups.find(g => g.id === group.id)
          ...
          : [group, ...s.groups],
        groupExpenses: { ...s.groupExpenses, [group.id]: payload.expenses ?? [] },
        syncingGroupId: null,
      }))
      return group
    } catch (e) {
  ...
  syncSharedGroup: async (groupId) => {
    ...
    try {
      ...
      const remote = await readSharedGroupFile(group.shareCode) as { group: Group; expenses: GroupExpense[]; syncedAt: number } | null
      const localExpenses = get().groupExpenses[groupId] ?? await groupExpenseQueries.getByGroup(groupId)
      ...
      for (const e of localExpenses) {
        ...
        if (!existing || e.createdAt > existing.createdAt) expenseMap.set(e.id, e)
      }
      ...
      for (const m of remoteMembers) if (!memberMap.has(m.id)) memberMap.set(m.id, m)
      const mergedGroup: Group = { ...group, members: Array.from(memberMap.values()), updatedAt: Date.now() }
      ...
      const payload = { group: mergedGroup, expenses: merged, syncedAt: Date.now() }
  ...
  unshareGroup: async (groupId) => {
    ...
    await groupQueries.update(groupId, { shareCode: undefined, isOwner: false })
    set(s => ({
      groups: s.groups.map(g => g.id === groupId ? { ...g, shareCode: undefined, isOwner: false } : g),
    }))
  },
}))
