...
const DEBIT_WORDS =
  /\b(debit(?:ed)?|spent|withdrawn|withdrawal|paid|purchase|charged|deducted|utilized)\b/i;
const CREDIT_WORDS =
  /\b(credit(?:ed)?|received|deposited|added|refund(?:ed)?|reversed|cashback)\b/i;
...
function parsePaymentMethod(text: string): PaymentMethod {
  ...
  if (
    /\bUPI\b|@[A-Z0-9]+\.(OK|YBL|ICICI|AXIS|SBI|HDFC|PAYTM|AIRTEL)\b/i.test(t)
  )
    return "upi";
  if (/\b(NEFT|RTGS|IMPS)\b/.test(t)) return "bank_transfer";
  if (/\b(CREDIT CARD|DEBIT CARD|CARD NO|CARD ENDING|CARD XX)\b/i.test(t))
    return "card";
...
const CATEGORY_KEYWORDS: [RegExp, string][] = [
  [
    /\b(ZOMATO|SWIGGY|DOMINOS?|MCDONALD|PIZZA|DUNZO|BLINKIT|ZEPTO|BIGBASKET|GROFER|DINEOUT|RELIANCERETAIL|JIOMART|RESTAURANT|CAFE|HOTEL|FOOD)\b/i,
    "Food & Dining",
  ],
  [
    /\b(UBER|OLA|RAPIDO|REDBUS|IRCTC|RAILWAY|METRO|AUTO|TAXI|FLIGHT|AIRLINE|INDIGO|SPICEJET|MAKE MY TRIP|CLEARTRIP)\b/i,
    "Transport",
  ],
  [
    /\b(AMAZON|FLIPKART|MYNTRA|MEESHO|NYKAA|AJIO|SHOPSY|SNAPDEAL|RELIANCE DIGITAL|CROMA|VIJAY SALES)\b/i,
    "Shopping",
  ],
  [
    /\b(NETFLIX|SPOTIFY|PRIME VIDEO|HOTSTAR|DISNEY|YOUTUBE|APPLE|GOOGLE PLAY|PLAYSTATION|XBOX|GAANA|JIO SAAVN|JIO)\b/i,
    "Subscriptions",
  ],
  [
    /\b(CINEMA|MOVIE|PVR|INOX|BOOKMYSHOW|THEATRE|CONCERT|EVENT)\b/i,
    "Entertainment",
  ],
  [
    /\b(HOSPITAL|CLINIC|PHARMACY|MEDICAL|MEDICINE|DOCTOR|APOLLO|FORTIS|MAX HEALTH|MANIPAL|LAB|DIAGNOSTIC|HEALTHKART)\b/i,
    "Health",
  ],
  [
    /\b(ELECTRICITY|MSEB|BESCOM|KSEB|BSES|TATA POWER|WATER|GAS|POSTPAID|BROADBAND|WIFI|JIOFIBER|AIRTEL|BSNL|TRAI)\b/i,
    "Utilities",
  ],
  [
    /\b(SCHOOL|COLLEGE|UNIVERSITY|COURSE|UDEMY|COURSERA|BYJU|UNACADEMY|TUITION|FEES|EXAM|GITHUB|CLAUDE|COPILOT)\b/i,
    "Education",
  ],
  ...
  [
    /\b(GYM|FITNESS|CULT|HEALTHIFY|SALON|SPA|PERSONAL CARE|GROOMING)\b/i,
    "Personal Care",
  ],
...
function scoreConfidence(
  result: Omit<ParsedSMS, "confidence">,
): ParsedSMS["confidence"] {
  let score = 0;
