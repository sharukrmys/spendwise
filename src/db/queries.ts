...
export const expenseQueries = {
  ...
  async getByCategory(categoryId: string): Promise<Expense[]> {
    return db.expenses.where('categoryId').equals(categoryId).reverse().sortBy('date')
  },

  async add(data: Omit<Expense, 'id' | 'createdAt' | 'updatedAt'>): Promise<Expense> {
    const now = Date.now()
  ...
  async getTotal(start?: number, end?: number): Promise<number> {
    const expenses = start && end ? await this.getByRange(start, end) : await this.getAll()
...
export const groupQueries = {
  ...
  async add(data: Omit<Group, 'id' | 'createdAt' | 'updatedAt'>): Promise<Group> {
    const now = Date.now()
    const group: Group = { ...data, id: generateId(), createdAt: now, updatedAt: now }
...
export const groupExpenseQueries = {
  async getByGroup(groupId: string): Promise<GroupExpense[]> {
    return db.groupExpenses.where('groupId').equals(groupId).reverse().sortBy('date')
  },

  async add(data: Omit<GroupExpense, 'id' | 'createdAt'>): Promise<GroupExpense> {
    const ge: GroupExpense = { ...data, id: generateId(), createdAt: Date.now() }
    await db.groupExpenses.add(ge)
    return ge
  },

  async update(id: string, data: Partial<GroupExpense>): Promise<void> {
    await db.groupExpenses.update(id, data)
  },
